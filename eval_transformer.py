...
from transformer_nld import SimpleTransformer
...
print(
    f"SBP MAE:      {np.mean(all_sbp_errors):.2f} ± {np.std(all_sbp_errors):.2f} mmHg"
)
print(
    f"DBP MAE:      {np.mean(all_dbp_errors):.2f} ± {np.std(all_dbp_errors):.2f} mmHg"
)
...
plt.tight_layout()
plt.savefig("transformer_eval.png", dpi=150)
plt.close()
print("\nSaved transformer_eval.png")
