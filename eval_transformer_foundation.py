...
from transformer_foundation_embeddings import DualStreamTransformer

FOUNDATION_CHECKPOINT = "transformer_foundation_checkpoint.pt"
...
def eval_foundation():
    ckpt = torch.load(FOUNDATION_CHECKPOINT, map_location=device, weights_only=False)
...
print("Evaluating Foundation transformer (PaPaGei Encoder)...")
foundation_sbp_err, foundation_dbp_err, foundation_cm = eval_foundation()
...
for name, sbp_e, dbp_e, cm in [
    ...
    ("Foundation transformer  (PPG + Poincaré + PaPaGei)", foundation_sbp_err, foundation_dbp_err, foundation_cm),
]:
    print(f"── {name} ──")
